...
#
# Tags look like `v<Outline version>-<release>`:
#
# - if defaults/main.yml points at an Outline version that has never been
#   released, the release counter restarts at 0 (`v1.9.2-0`)
# - otherwise the counter is incremented (`v1.9.2-1`), but only if something
...
version="$(sed -nE 's|^outline_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Outline version from $defaults_path"
	exit 1
fi

# The role's version values carry no leading `v` (e.g. `1.9.2`), while the tags
# do (`v1.9.2-0`). Stripping first keeps this correct either way.
